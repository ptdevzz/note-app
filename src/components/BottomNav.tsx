...
import { Home, Bookmark, CalendarDays, Gift, Plus } from 'lucide-react';

export type TabType = 'home' | 'tiktok' | 'plan' | 'coupons';
  ...
  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-md bg-slate-900/95 backdrop-blur-lg border-t border-slate-800 px-3 py-2 z-40">
      <div className="flex justify-around items-center relative">
        ...
        >
          <Gift className={`w-5 h-5 ${activeTab === 'coupons' ? 'stroke-[2.5px]' : 'stroke-[1.75px]'}`} />
          <span className="text-[11px] mt-1 font-medium tracking-tight">Đặc Quyền</span>
        </button>
      </div>
    </nav>
  );
