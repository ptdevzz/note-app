...
export default function TikTokAddModal({ isOpen, onClose, onAdd }: TikTokAddModalProps) {
  ...
  const handleParseUrl = async (rawUrl: string) => {
    ...
    try {
      ...
      setPreview({
        title: d.title || 'Địa điểm TikTok hot',
        thumbnail: d.thumbnail || 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?auto=format&fit=crop&w=600&q=80',
        authorName: d.authorName || d.author_name || 'TikTok User',
        guessedCategory: (d.category || d.guessedCategory || 'Ăn tối') as PlaceCategory,
        tags: d.tags || ['Sài Gòn'],
      });
    } catch (err) {
        ...
        try {
          ...
          if (d && d.title) {
            finalPreview = {
              ...
              authorName: d.authorName || d.author_name,
              guessedCategory: (d.category || d.guessedCategory || 'Ăn tối') as PlaceCategory,
              tags: d.tags || ['Sài Gòn'],
            };
          }
